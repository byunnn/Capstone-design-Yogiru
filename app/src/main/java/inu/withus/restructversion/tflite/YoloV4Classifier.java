...
package inu.withus.restructversion.tflite;
...
public class YoloV4Classifier implements Classifier {
    ...
    //non maximum suppression(nms) : 동일한 object에 여러개의 bbox가 있을 때, 가장 스코어가 높은 박스만 남기고 나머지를 제거하는 것
    protected ArrayList<Recognition> nms(ArrayList<Recognition> list) {
        ArrayList<Recognition> nmsList = new ArrayList<Recognition>();

        for (int k = 0; k < labels.size(); k++) {
            //1.find max confidence per class  각 클래스 당 가장 정확도가 높은 것 찾기
            ...
            //2.do non maximum suppression(nms) : a class of algorithms to select one entity (e.g., bounding boxes) out of many overlapping entities
            while (pq.size() > 0) {
    ...
    // IoU(Intersection over Union) : 객체 인식분야에서 예측 bbox과 ground truth가 일치하는 정도를 0과 1사이의 값으로 나타낸 값
    ...
    private ArrayList<Recognition> getDetectionsForTiny(ByteBuffer byteBuffer, Bitmap bitmap) {
        ...
        for (int i = 0; i < gridWidth;i++){
            ...
            if (score > getObjThresh()){
                ...
                //detections은 REcognition class가 모인 배열 => new Recognition() 생성자로 배열에 Recognition class 추가해 주는 과정
                detections.add(new Recognition("" + i, labels.get(detectedClass),score,rectF,detectedClass ));
...
//        }
        ...
        LOGGER.i("recognition ::::::::::::::::::::::: " + recognitions);
        for(int i=0; i<recognitions.size(); i++){
            String obj_title = recognitions.get(i).getTitle();
            LOGGER.i("-----------title------------- " + obj_title);
        }
